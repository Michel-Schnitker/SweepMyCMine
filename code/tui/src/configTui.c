...
static bool requiredNumberStream = false;
...
void drawConfigWindow(uint32_t windowRow, uint32_t windowCol, WINDOW *window){
    ...
    if(requiredNumberStream){
        ...
        switch (focusFeature) {
            ...
            case CUSTOMIZE_X_SIZE:
                config->customizeFieldConfig.xSize = newNumber;
                break;

            case CUSTOMIZE_Y_SIZE:
                config->customizeFieldConfig.ySize = newNumber;
                break;

            case CUSTOMIZE_BOMBS:
                config->customizeFieldConfig.bombs = newNumber;
                break;
